<template>
  <div class="container mx-auto pt-28 pb-20">
    <h1 class="text-5xl font-bold mb-5">예금 상품 비교</h1>
    <p class="text-xl text-muted-foreground text-gray-500">
      다양한 은행의 예금 상품을 비교해보세요. 최적의 금리와 조건을 찾아보세요!
    </p>
    ...
    <div v-if="viewMode === 'card'" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
      <div
        ...
        :key="item.id"
        class="border rounded-lg p-4 shadow-sm hover:shadow-md"
      >
        <router-link :to="{ name: 'deposit-detail', params: { id: item.id } }">
      ...
      <tbody>
        <tr v-for="item in filteredList" :key="item.id">
          <td class="border px-2 py-1">
            <router-link
              :to="{ name: 'deposit-detail', params: { id: item.id } }"
...
// 데이터 로딩
onMounted(async () => {
  const res = await axios.get('http://127.0.0.1:8000/api/products/deposits/')
...
function isScrapped(item) {
  return scraps.value.some((s) => s.deposit?.id === item.id)
}

// 스크랩 추가/취소
async function toggleScrap(item) {
  const scrap = scraps.value.find((s) => s.deposit?.id === item.id)
  if (scrap) {
