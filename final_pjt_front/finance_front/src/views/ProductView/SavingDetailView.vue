<template>
  <div v-if="saving" class="container mx-auto pt-28 pb-20">
    <router-link :to="{ name: 'products_saving' }" class="text-sm text-gray-600 hover:underline">
      ← 적금 상품 목록으로 돌아가기
    </router-link>

    <div class="flex justify-between items-start mt-4">
      <div>
        <h2 class="text-3xl font-bold mb-1">{{ saving.name }}</h2>
        <p class="text-gray-500 mb-4">{{ saving.bank.name }}</p>
      </div>
      <div class="text-right">
        <p class="text-red-500 text-2xl font-bold">{{ getMaxRate(saving) }}%</p>
    ...
    <div class="mt-6">
      ...
      <table class="table-auto w-full border">
        ...
        <tbody>
          <tr v-for="opt in saving.options" :key="opt.id">
    ...
    <div class="bg-white border rounded-lg p-6 mt-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          ...
          <p class="text-base">{{ saving.bank.name }}</p>
        </div>
        <div>
          <p class="text-sm text-gray-500 mb-1">금리 정보</p>
          <p class="text-base">세전 {{ getMaxRate(saving) }}% / 세후 {{ (getMaxRate(saving) * 0.845).toFixed(2) }}%</p>
        </div>
        <div>
          <p class="text-sm text-gray-500 mb-1">가입 기간</p>
          <p class="text-base">{{ getMaxTerm(saving) }}개월</p>
        </div>
      </div>
    </div>
    ...
    <div class="border rounded-lg p-6 mt-6">
      ...
      <div class="mb-4">
        ...
        <input v-model.number="principal" type="number" step="10000" class="border rounded px-2 py-1 w-48" placeholder="예: 1000000" />
      </div>
      <div class="grid grid-cols-3 text-center">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-green-600">+{{ formatCurrency(getEstimatedInterest()) }}</p>
        </div>
        <div>
    ...
    <div class="border rounded-lg p-4 mt-6">
      ...
      <ul class="list-disc list-inside text-sm text-gray-600 space-y-1">
        <li>적금자보호법에 따라 적금보험공사가 보호합니다.</li>
...
onMounted(async () => {
  try {
    const res = await axios.get(`http://127.0.0.1:8000/api/products/savings/${route.params.id}/`)
...
function drawChart() {
  if (!saving.value || !Array.isArray(saving.value.options)) return
  const ctx = document.getElementById('rateChart')
  if (!ctx) return
  const labels = saving.value.options.map(opt => `${opt.save_trm}개월`)
  const data = saving.value.options.map(opt => opt.intr_rate2)
  new Chart(ctx, {
    type: 'bar',
    data: {
      labels,
...
const isScrapped = computed(() => {
  ...
  return scraps.value.some(s => s.saving?.id === saving.value.id)
})

async function toggleScrap() {
  if (!saving.value) return
  const scrap = scraps.value.find(s => s.saving?.id === saving.value.id)
  try {
    if (scrap) {
      ...
      await axios.post('http://127.0.0.1:8000/api/products/scraps/', {
        saving_id: saving.value.id
      }, {
        headers: { Authorization: `Token ${localStorage.getItem('access_token')}` }
      })
    }
...
function getEstimatedInterest() {
  if (!saving.value || !saving.value.options?.length) return 0
  const options = saving.value.options
