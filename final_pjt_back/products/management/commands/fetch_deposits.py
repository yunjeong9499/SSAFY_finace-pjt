...
from products.models import Deposit, DepositOption, Bank
...
BANK_GROUPS = ["020000", "030300", "050000", "060000", "070000"]  # 다양한 은행권

class Command(BaseCommand):
    help = "Fetch ALL deposit products from FSS for all bank groups"
    ...
    def handle(self, *args, **kwargs):
        ...
        for group_code in BANK_GROUPS:
            page = 1
            self.stdout.write(self.style.NOTICE(f"📂 {group_code} 은행권 처리 시작"))
            ...
            while True:
                ...
                for opt in option_list:
                    print("📌 금리 저장 중:", opt.get('fin_prdt_cd'), opt.get('save_trm'),
                          opt.get('intr_rate'), opt.get('intr_rate2'))

                    deposit = Deposit.objects.get(deposit_code=opt['fin_prdt_cd'])
                ...
                self.stdout.write(self.style.SUCCESS(f"✅ [{group_code}] 페이지 {page} 처리 완료"))
                page += 1

        self.stdout.write(self.style.SUCCESS(f"🎉 전체 은행권 예금 상품 {total_saved}개 저장 완료"))
